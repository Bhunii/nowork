@extends('layouts.nav.instructor',['title' => 'Update Data'])
...
@section('content_profile')
    <style>
        ...
        .form_update_instructor::-webkit-scrollbar {
            width: 8px;
            height: 5px;
        }
...
<main class="contenido_form">
    <form class="form form_update_instructor" method="post" action="{{ route('instructor.update') }}">
        @csrf
        @method('PUT')
        <div>
        ...
        <div>
            ...
            name="speciality"
            type="text"
            value="{{ auth()->user()->instructor->speciality}}"
            required
            >
            @error('speciality')
                <small>{{$message}}</small>
